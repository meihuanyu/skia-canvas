        ...
        switch (match) {
          ...
          case "size":
            ...
            let [emSize, leading] = splitBy(token, '/'),
                ...
                features = font.variant == 'small-caps' ? {
              on: ['smcp', 'onum']
            } : {},
                {
              style,
              stretch,
              variant
            } = font; // make sure all the numeric fields have legitimate values

            let invalid = !isFinite(size) ? "font size \"".concat(emSize, "\"") : !isFinite(lineHeight) ? "line height \"".concat(leading, "\"") : !isFinite(weight) ? "font weight \"".concat(font.weight, "\"") : family.length == 0 ? "font family \"".concat(tokens.join(', '), "\"") : false;
            ...
            if (!invalid) {
              ...
              return cache.font[str] = Object.assign(font, {
                ...
                family,
                features,
                canonical: [style, variant !== style && variant, [variant, style].indexOf(weight) == -1 && weight, [variant, style, weight].indexOf(stretch) == -1 && stretch, "".concat(size, "px/").concat(lineHeight, "px"), family.map(nm => nm.match(/\s/) ? "\"".concat(nm, "\"") : nm).join(", ")].filter(Boolean).join(' ')
              });
            }

            throw new Error("Invalid ".concat(invalid));

          default:
            throw new Error("Unrecognized font attribute \"".concat(token, "\""));
        }
      }
...
function parseSize(str) {
  let emSize = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 16;

  if (m = numSizeRE.exec(str)) {
    let [size, unit] = [parseFloat(m[1]), m[2]];
    return size * (unit == 'px' ? 1 : unit == 'pt' ? 1 / 0.75 : unit == '%' ? emSize / 100 : unit == 'pc' ? 16 : unit == 'in' ? 96 : unit == 'cm' ? 96.0 / 2.54 : unit == 'mm' ? 96.0 / 25.4 : unit == 'q' ? 96 / 25.4 / 4 : unit.match('r?em') ? emSize : NaN);
  }
    ...
    for (let token of splitBy(str, /\s+/)) {
      if (token == 'normal') {
        ...
        featureMap[token].forEach(feat => {
          if (feat[0] == '-') features.off.push(feat.slice(1));else features.on.push(feat);
        });
        variants.push(token);
      } else if (m = parameterizedRE.exec(token)) {
        let subPattern = alternatesMap[m[1]],
            subValue = Math.max(0, Math.min(99, parseInt(m[2], 10))),
            [feat, val] = subPattern.replace(/##/, subValue < 10 ? '0' + subValue : subValue).replace(/#/, Math.min(9, subValue)).split(' ');
        if (typeof val == 'undefined') features.on.push(feat);else features[feat] = parseInt(val, 10);
        variants.push("".concat(m[1], "(").concat(subValue, ")"));
      } else {
        throw new Error("Invalid font variant \"".concat(token, "\""));
      }
    }
  ...
  for (var spec of splitBy(str, /\s+/) || []) {
    if (m = shadowFilterRE.exec(spec)) {
      ...
      if (dims.length == 3 && !!color) {
        ...
        canonical.push("".concat(kind, "(").concat(lengths.join(' '), " ").concat(color.replace(/ /g, ''), ")"));
      }
      ...
      if (isFinite(val)) {
        filters[kind] = val;
        canonical.push("".concat(kind, "(").concat(arg.trim(), ")"));
      }
    }
  }
