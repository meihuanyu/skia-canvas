...
function options(pages) {
  ...
  return function (format, archive) {
    var {
      fromMime,
      toMime,
      expected
    ...
    if (!format) throw new Error("Unsupported file format \"".concat(ext, "\" (expected ").concat(expected, ")"));
    ...
    if (isFinite(idx) && idx < 0 || idx >= pp) throw new RangeError(pp == 1 ? "Canvas only has a \u2018page 1\u2019 (".concat(idx, " is out of bounds)") : "Canvas has pages 1\u2013".concat(pp, " (").concat(idx, " is out of bounds)"));
...
class Zip {
  ...
  async add(filename, blob) {
    let folder = !blob,
        name = Zip.encoder.encode("".concat(this.directory, "/").concat(folder ? '' : filename)),
  ...
  toBuffer() {
    ...
    let length = this.files.reduce((len, _ref) => {
      let {
        name
      } = _ref;
      return 46 + name.length + len;
    }, 0),
...
_defineProperty(Zip, "encoder", new TextEncoder());
...
const asBuffer = function asBuffer() {
  return asBlob(...arguments).then(b => b.arrayBuffer());
};
...
const asZipDownload = async (pages, mime, quality, matte, archive, pattern, padding) => {
  ...
  _download("".concat(folder, ".zip"), zip.blob);
};
