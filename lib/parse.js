...
var cursorStyleRE = /^(((vertical-)?text)|(grab(bing)?)|((n|e|w|s|ne|nw|se|sw|ns|ew|nesw|nwse|row|col)-resize)|(zoom-(in|out))|alias|all-scroll|arrow|cell|context-menu|copy|crosshair|default|hand|help|move|no-drop|not-allowed|progress|wait|none)$/
function parseCursor(str){
  let m = cursorStyleRE.exec((str||"").toLowerCase().trim())
  return m ? m[0] : null
}
...
function parseOutput(pages, {filename='', extension, format, page, quality, density, fonts}={}){
  var ext = format || extension || extname(filename),
  ...
  let padding, pattern = filename.replace(/{(\d*)}/g, (_, width) => {
  ...
  if (density===undefined){
    let m = basename(filename, ext).match(/@(\d+)x$/i)
  ...
  if (fonts===undefined){
    ...
    if (format != 'svg'){
      throw new TypeError(`The fonts option is only valid for fonts in svg images (not ${format})`)
    }else{
      fonts = !!fonts
    }
  }
  ...
  return {filename, pattern, format, mime, page:idx, padding, quality, density, fonts}
}
