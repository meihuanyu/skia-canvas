...
class Window extends EventEmitter{
  constructor(canvas, {title="Skia Canvas", background="rgba(16,16,16,0.75)", fullscreen=false, fps, frame, cursor}={}){
  ...
  display(){
    ...
    const dispatch = (...payload) => {
      ...
      if (input!==undefined){
        let type = 'input'
        this.emit(type, {type, value:input, code:input.charCodeAt()})
      }
    ...
    if (!this.state.active){
      ...
      let fps = this.state.loop ? this.state.fps : 0,
          {cursor, background} = this.state
