...
const crate = require('./v6'),
      ...
      {parseFont, parseVariant, parseFilter, parseCursor} = require('./parse')
...
class Window extends EventEmitter{
  constructor(canvas, {title="", background="rgba(16,16,16,0.75)", fullscreen, fps, frame, cursor}={}){
    canvas = (canvas instanceof Canvas) ? canvas : new Canvas(300, 150)
    fps = typeof fps=='number' ? Math.max(0, Math.floor(fps)) : 60
    frame = typeof frame=='number' ? Math.floor(fps) : 0
    cursor = parseCursor(cursor||'') || "default"
    fullscreen = !!fullscreen

    let {width, height} = canvas,
        state = {fullscreen, frame, fps, background, cursor, active:false, loop:undefined}
    Object.assign(super(), {state, canvas, width, height, title})
  }
  ...
  display(){
    ...
    const dispatch = (...payload) => {
      ...
      if (keyEvent!==undefined){
        ...
        if (keyEvent=='keydown' && key=='Q' && metaKey && !repeat){
          this.state.active = false
        }else if (keyEvent=='keydown' && key=='F' && metaKey && !repeat){
    ...
    if (!this.state.active){
      ...
      crate.display(
        core(Canvas.contexts.get(this.canvas)[0]), dispatch, animate, this.state.background
      )
      this.state.active = false
    }
  }
  ...
  [REPR](depth, options) {
    let {title, x, y, width, height, canvas} = this
    return `Window ${inspect({title, x, y, width, height, canvas}, options)}`
  }
