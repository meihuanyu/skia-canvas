...
class Window extends EventEmitter{
  ...
  display(){
    const dispatch = (...payload) => {
      var [x, y, width, height, fullscreen, keyEvent, ...keyInfo]  = payload
      ...
      if (keyEvent!==undefined){
        let [key, code, repeat, altKey, ctrlKey, metaKey, shiftKey] = keyInfo,
            info = {key, code, repeat, altKey, ctrlKey, metaKey, shiftKey}
        this.emit(keyEvent, info)
      }

      var {x, y, width, height, title} = this,
          {active, fullscreen} = this.state,
          ctx = core(Canvas.contexts.get(this.canvas)[0])

      return [ctx, title, active, fullscreen, x, y, width, height]
    }
