...
class Window extends EventEmitter{
  ...
  display(){
    const dispatch = (e, ...args) => {
      // console.log('event', e)
      const rej = () => {}

      if (e=='error') return rej()
      else if (e=='resize'){
        ...
        args = args[0]
        this.state.fullscreen = args
      }else if (e.match(/^key/)){
        let [key, code, repeat, altKey, ctrlKey, metaKey, shiftKey] = args
        args = {key, code, repeat, altKey, ctrlKey, metaKey, shiftKey}
      }

      this.emit(e, args)

      let {x, y, width, height, title} = this,
          {active, fullscreen} = this.state,
          ctx = core(Canvas.contexts.get(this.canvas)[0])

      return (e=='fullscreen') ? [ctx, title, active, null, null, null, null, null]
            : (e=='resize') ? [ctx, title, active, fullscreen, null, null, null, null]
            : (e=='move') ? [ctx, title, active, fullscreen, null, null, width, height]
            : [ctx, title, active, fullscreen, x, y, width, height]
    }

    const animate = (e, frame) => {
      this.emit("frame", frame)
      return core(Canvas.contexts.get(this.canvas)[0])
    }

    // this will block until the window is closed, but events from the window will still be dispatched
    this.state.active = true
    crate.display(core(Canvas.contexts.get(this.canvas)[0]), this.title, dispatch, animate)
  }
  ...
  fullscreen(mode){
    ...
    console.log(`full?`, this.state.fullscreen)
  }
}
