...
class Window extends EventEmitter{
  static kwargs = "x,y,width,height,title,page,background,fullscreen,fps,frame,cursor,visible".split(/,/)
  ...
  static feedback(win, response){
    let {title, active, fullscreen, loop, fps, width, height, x, y, cursor, visible} = win.state
    let frameRate = loop ? fps : 0
    response.splice(0, response.length,
      win.canvas.pages[win.page-1][ø],
      title, active, fullscreen, frameRate, width, height, x, y, cursor, visible
    )
    return response
  }

  display(){
    let c2d = canvas => canvas.pages[this.page-1][ø],
        response = []
    ...
    const dispatch = (...payload) => {
      ...
      return Window.feedback(this, response)
    }
    ...
    if (!this.state.active){
      let {background, width, height, loop, visible} = this.state
      ...
      crate.displayWindow(c2d(this.canvas), dispatch, animate, background, width, height)
      this.state.active = false
    }
  }
