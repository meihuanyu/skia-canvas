...
class Canvas extends RustClass{
  ...
  set width(w){
    ...
    if (ctx) ctx.ƒ('resetSize', core(this))
  }
  ...
  saveAs(filename, opts={}){
    ...
    let contexts = Canvas.contexts.get(this).map(core),
        {format, quality, padding, pattern, density, fonts} = parse.output(contexts, opts),
        args = [contexts, pattern, padding, format, quality, density, fonts];

    if (this.async){
      let worker = new EventEmitter(),
          callback = (result, msg) => worker.emit(result, msg)
      this.ƒ("save", callback, ...args)
  ...
  toBuffer(extension="png", opts={}){
    ...
    let contexts = Canvas.contexts.get(this),
        {format, quality, page, density, fonts} = parse.output(contexts, opts),
        pages = isFinite(page) ? contexts.slice(page, page+1)
              : format=='pdf' ? contexts
              : contexts.slice(0, 1),
        args = [pages.map(core), format, quality, density, fonts]

    if (this.async){
      let worker = new EventEmitter(),
          callback = (result, msg) => worker.emit(result, msg)
      this.ƒ("toBuffer", callback, ...args)
      return new Promise((res, rej) => worker.once('ok', res).once('err', msg => rej(new Error(msg))) )
    }else{
  ...
  toDataURL(extension="png", opts={}){
    opts.extension = extension
    let contexts = Canvas.contexts.get(this),
        {format, mime, quality, density, fonts} = parse.output(contexts, opts)
        urlify = data => `data:${mime};base64,${data.toString('base64')}`,
        buffer = this.toBuffer({format, quality, page, density, fonts})

    if (this.async){
      return buffer.then(urlify)
    }else{
      return urlify(buffer)
    }
  }

  [REPR](depth, options) {
    let {width, height, async} = this
    return `Canvas ${inspect({width, height, async}, options)}`
  }
...
class Window extends EventEmitter{
  constructor(canvas, args={}){
    let kwargs = "x,y,width,height,title,background,fullscreen,fps,frame,cursor".split(/,/)
  ...
  display(){
    let c2d = canvas => ((Canvas.contexts.get(canvas) || [])[0] || {})[ø],
        response = []
