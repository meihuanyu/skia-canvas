...
class Window extends EventEmitter{
  ...
  display(){
    ...
    const dispatch = (...payload) => {
      let [x, y, width, height, fullscreen, input, keyEvent, ...keyInfo]  = payload
      ...
      if (input!==undefined){
        this.emit("input", input, input.charCodeAt())
      }

      if (keyEvent!==undefined){
        let [key, code, repeat, altKey, ctrlKey, metaKey, shiftKey] = keyInfo
        this.emit(keyEvent, {key, code, repeat, altKey, ctrlKey, metaKey, shiftKey})
      }
