...
class Canvas extends RustClass{
  ...
  getContext(kind){
    // allocates a first context if none exists
  ...
  get pages(){
    // causes lazy allocation as a side effect
    let contexts = Canvas.contexts.get(this).slice().reverse()
    return contexts.length ? contexts : [this.getContext("2d")]
  }
  ...
  saveAs(filename, opts={}){
    ...
    let {format, quality, pages, padding, pattern, density, outline} = io.options(this.pages, {filename, ...opts}),
  ...
  toBuffer(extension="png", opts={}){
    opts = typeof opts=='number' ? {quality:opts} : opts
    let {format, quality, pages, density, outline} = io.options(this.pages, {extension, ...opts}),
...
class CanvasPattern extends RustClass{
  constructor(src, repeat){
    if (src instanceof Image){
      ...
      super().init('from_canvas', core(src.getContext("2d")), repeat)
    }else{
...
class CanvasRenderingContext2D extends RustClass{
  ...
  set font(str){             this.ƒ('set_font', css.font(str)) }
  ...
  set fontVariant(str){      this.ƒ('set_fontVariant', css.variant(str)) }
  ...
  set filter(str){            this.ƒ('set_filter', css.filter(str)) }
...
class Window extends EventEmitter{
  static kwargs = "x,y,width,height,title,page,background,fullscreen,fps,frame,cursor,fit,visible".split(/,/)

  constructor(canvas={}, args={}){
    // accept ƒ(canvas, {…}) or ƒ({…})
    [canvas, args] = (canvas instanceof Canvas) ? [canvas, args]
                   : [new Canvas(canvas.width || 300, canvas.height || 150), canvas]
    ...
    Object.assign(this, Object.fromEntries(
      Object.entries(args).filter(([k, v]) => Window.kwargs.includes(k) && v!==undefined)
    ))
  }
  ...
  set fit(val){
    let style = parse.fit(val)
    if (style){
      this.state.fit = style
    }
    else throw new TypeError("Invalid CSS object-fit value")
  }
  ...
  set visible(val){ this.state.visible = !!val }

  display(){
    let response = []
    const feedback = () => {
      response.splice(0, response.length,
        this.canvas.pages[this.page-1][ø],
        this.state.title,
        this.state.active,
    ...
    if (!this.state.active){
      let context = core(this.canvas.pages[this.page-1]),
          {background, width, height, loop, visible} = this.state
      this.state.loop = loop===undefined ? this.listenerCount("frame") > 0 : loop
      this.state.visible = visible===undefined ? true : visible
      this.state.active = true

      // this call will block until the window is closed, but events from the window will still be dispatched
      crate.displayWindow(context, dispatch, animate, background, width, height)
      this.state.active = false
    }
  }
  ...
  [REPR](depth, options) {
    let {title, x, y, width, height, cursor, visible, fullscreen, fps, background, fit, page, canvas} = this
    return `Window ${inspect({x, y, width, height, visible, fullscreen, fps, title, cursor, background, fit, page, canvas}, options)}`
  }
